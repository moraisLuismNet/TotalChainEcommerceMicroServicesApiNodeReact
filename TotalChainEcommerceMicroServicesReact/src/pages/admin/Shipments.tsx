...
const Shipments = () => {
  ...
  const filtered = shipments.filter((s) => {
    ...
    return (
      ...
      new Date(s.createdAt).toLocaleDateString().toLowerCase().includes(term)
    );
  });
  ...
  return (
    <div className="container mt-4">
      ...
      <div className="mb-3">
        <input type="text" className="form-control" placeholder="Search by user or date..."
          value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} />
      </div>
      <table className="table table-hover">
        <thead>
          <tr><th>Date</th><th>User</th><th>Status</th><th></th></tr>
        </thead>
        <tbody>
          {currentItems.map((s) => (
            <tr key={s.id}>
              <td className="text-nowrap">{new Date(s.createdAt).toLocaleDateString()}</td>
              <td>{s.userEmail}</td>
              <td>
