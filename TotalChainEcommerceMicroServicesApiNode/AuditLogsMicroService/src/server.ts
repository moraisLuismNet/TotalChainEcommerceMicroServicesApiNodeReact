...
app.use("/api-docs", swaggerUi.serve, swaggerUi.setup(swaggerSpec, { explorer: true, customSiteTitle: "AuditLogsMicroService", swaggerOptions: { persistAuthorization: true } }));

app.use("/api/audit-logs", auditLogRoutes);

app.use(errorHandler);
async function start() { try { await sequelize.authenticate(); await sequelize.sync(); app.listen(PORT, () => { console.log(`AuditLogsMicroService on port ${PORT}`); }); } catch (e) { console.error("Start error:", e); process.exit(1); } } start();
export default app;
