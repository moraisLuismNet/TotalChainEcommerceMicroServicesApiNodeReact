...
import shipmentRoutes from "./api/routes/shipmentRoutes";
...
app.use("/api-docs", swaggerUi.serve, swaggerUi.setup(swaggerSpec, { explorer: true, customSiteTitle: "ShipmentsMicroService", swaggerOptions: { persistAuthorization: true } }));
app.use("/api/shipments", shipmentRoutes);
app.use(errorHandler);
async function start() { try { await sequelize.authenticate(); await sequelize.sync(); app.listen(PORT, () => { console.log(`ShipmentsMicroService on port ${PORT}`); }); } catch (e) { console.error("Start error:", e); process.exit(1); } } start();
export default app;
